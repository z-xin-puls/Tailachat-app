...
## 2026-05-14 功能修复与代码优化

### 修复的问题

本次修复针对项目中的多个功能性和结构性问题进行了处理：

| 问题类型 | 问题描述 | 影响范围 |
|---------|---------|---------|
| **功能缺陷** | 禁言功能无效，被禁言用户仍可发送消息 | `app.py` |
| **功能缺陷** | 踢人功能清理不彻底，被踢用户残留于数据结构 | `app.py` |
| **导入缺失** | 缺少 `AuthorizationError` 导入，权限错误无法正确抛出 | `routes/profile.py` |
| **游标混用** | 数据库游标混用（普通游标/字典游标）导致数据访问错误 | `routes/main.py`, `routes/profile.py` |
| **重复定义** | `room_users` 变量重复定义，导致数据不一致 | `app.py` |
| **前端遗漏** | 房间初始化时未检查禁言状态 | `static/js/room.js` |

### 修复内容

#### 1. 禁言功能修复
在 `handle_send_chat_message` 函数开头添加禁言状态检查，拦截被禁言用户的消息发送。
...
#### 2. 踢人功能优化
优化 `handle_kick_user` 函数，确保被踢用户从 `socket_room_users` 和 `username_to_sid` 等数据结构中彻底移除。

#### 3. 数据库游标统一
将相关函数的游标统一改为 `dictionary=True`，并调整数据访问方式：
- 从索引访问：`r[0]` → 改为字典键访问：`r['id']`

#### 4. 重复定义移除
移除 `app.py` 中第一次重复定义的 `room_users` 变量。

#### 5. 前端禁言检查
在房间初始化时添加主动检查禁言状态的逻辑。
...
本次重构和修复成功地将Flask项目的前后端代码分离，并修复了多个功能和结构性问题：
